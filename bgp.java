...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener, ILinkDiscoveryListener,
	...
	public IPv4Address privateGatewayIp;
	
	private HashMap<DatapathId, HashMap<IPv4Address,OFPort>> hostMap;
	...
	/*public proxyArpAction(OFPacketOutBuilder ofp){
	...
	*/
	public Command proxyArpHandlePacketIn(IOFSwitch sw,ARP arp,OFPort inPort){
		...
		ARP repArp = new ARP();
		repArp.setOpCode(ARP.OP_REPLY);
		repArp.setHardwareType(ARP.HW_TYPE_ETHERNET);
		repArp.setProtocolType(ARP.PROTO_TYPE_IP);
		repArp.setSenderHardwareAddress(gatewayMac);
		repArp.setSenderProtocolAddress(privateGatewayIP);
		repArp.setTargetHardwareAddress(arp.getSenderHardwareAddress());
		repArp.setTargetProtocolAddress(arp.getSenderProtocolAddress());
		ethernet.setPayload(repArp);
	...
	public boolean addLocation(DatapathId dpid,IPv4Address addr,OFPort port){
		HashMap<IPv4Address,OFPort> tmpMap  = hostMap.get(dpid);
		if(internalMap == null){
			tmpMap = new HashMap<>();
			hostMap.put(dpid,internalMap);
		}
		if(tmpMap.containsKey(addr)) return false;
		tmpMap.put(addr,port);
		return true;
	}
	public boolean presentSwitch(DatapathId id){
		return hostMap.containsKey(id);
	}
	public boolean containsIP(DatapathId dpid,IPv4Address addr){
		HashMap<IPv4Address,OFPort> internalMap = hostMap.get(dpid);
		if(tmpMap == null) return false;
		return tmpMap.containsKey(addr);
	}
	public OFPort getLocation(DatapathId dpid,IPv4Address addr){
		HashMap<IPv4Address,OFPort> tmpMap = hostMap.get(dpid);
		if(tmpMap == null) return null;
		return tmpMap.get(addr);
	}
	}
